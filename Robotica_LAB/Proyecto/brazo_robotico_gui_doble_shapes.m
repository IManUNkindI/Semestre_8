...
function iniciarEnvioAutomatico(brazo_id)
    if brazo_id == 1
        if ~isempty(s1) && isvalid(s1) && ~envio_automatico_activo1
            ...
            start(timer_envio1);
            uialert(fig, 'Envío automático activado para Brazo 1 (cada 0.2s)', 'Info');
        end
    else
        if ~isempty(s2) && isvalid(s2) && ~envio_automatico_activo2
            ...
            start(timer_envio2);
            uialert(fig, 'Envío automático activado para Brazo 2 (cada 0.2s)', 'Info');
        end
    end
end
...
function detenerEnvioAutomatico(brazo_id)
    if brazo_id == 1
        if envio_automatico_activo1 && ~isempty(timer_envio1)
            ...
            timer_envio1 = [];
            envio_automatico_activo1 = false;
            uialert(fig, 'Envío automático desactivado para Brazo 1', 'Info');
        end
    else
        if envio_automatico_activo2 && ~isempty(timer_envio2)
            ...
            timer_envio2 = [];
            envio_automatico_activo2 = false;
            uialert(fig, 'Envío automático desactivado para Brazo 2', 'Info');
        end
    end
end
    ...
    if brazo_id == 1
        if ~isempty(s1) && isvalid(s1) && envio_automatico_activo1
            ...
            try
                ...
                uialert(fig, ['Error en envío automático Brazo 1: ' ME.message], 'Error');
            end
        end
    else
        if ~isempty(s2) && isvalid(s2) && envio_automatico_activo2
            ...
            try
                ...
                uialert(fig, ['Error en envío automático Brazo 2: ' ME.message], 'Error');
            end
        end
    end
end
...
function movimientoSuave(brazo_id, q_inicial, q_final, duracion)
    ...
    for i=1:N
        ...
        if brazo_id == 1
            ...
            % El envío automático se encarga de enviar los ángulos
        else
...
function botonGraficasCallback()
    ...
    uialert(fig,'Sistema de gráficas activado. Las gráficas se actualizarán en tiempo real.','Info');
end
