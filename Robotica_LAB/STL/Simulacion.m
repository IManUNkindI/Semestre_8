...
L(1) = Link('d', 100, 'a', 0.00, 'alpha',  pi/2, 'qlim', [-pi,  pi]); % J1
L(2) = Link('d', 0.00, 'a', 100, 'alpha',  0.00, 'qlim', [0,  pi/2]);   % J2
L(3) = Link('d', 0.00, 'a', 0.00, 'alpha',  pi/2, 'qlim', [-pi/2,  pi/4]);   % J3
L(4) = Link('d', 100, 'a', 0.00, 'alpha',  pi/2, 'qlim', [-pi/2,  pi/2]); % J4
L(5) = Link('d', 50, 'a', 0.00, 'alpha',  pi/2, 'qlim', [-pi/2,  pi/2]);   % J5
...
% Visualizar el robot
figure;
robot.plot3d(q0, 'workspace', [-1 1 -1 1 0 1]);
camzoom(6)  % Aumenta el zoom 2x
